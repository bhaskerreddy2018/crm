/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { IsString, IsOptional, ValidateNested, IsDate } from "class-validator";
import { ApplicationUpdateManyWithoutStudentsInput } from "./ApplicationUpdateManyWithoutStudentsInput";
import { Type } from "class-transformer";
import { AppointmentUpdateManyWithoutStudentsInput } from "./AppointmentUpdateManyWithoutStudentsInput";
import { RoadMapUpdateManyWithoutStudentsInput } from "./RoadMapUpdateManyWithoutStudentsInput";

@InputType()
class StudentUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  address?: string | null;

  @ApiProperty({
    required: false,
    type: () => ApplicationUpdateManyWithoutStudentsInput,
  })
  @ValidateNested()
  @Type(() => ApplicationUpdateManyWithoutStudentsInput)
  @IsOptional()
  @Field(() => ApplicationUpdateManyWithoutStudentsInput, {
    nullable: true,
  })
  applications?: ApplicationUpdateManyWithoutStudentsInput;

  @ApiProperty({
    required: false,
    type: () => AppointmentUpdateManyWithoutStudentsInput,
  })
  @ValidateNested()
  @Type(() => AppointmentUpdateManyWithoutStudentsInput)
  @IsOptional()
  @Field(() => AppointmentUpdateManyWithoutStudentsInput, {
    nullable: true,
  })
  appointments?: AppointmentUpdateManyWithoutStudentsInput;

  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  dateOfBirth?: Date | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  phoneNumber?: string | null;

  @ApiProperty({
    required: false,
    type: () => RoadMapUpdateManyWithoutStudentsInput,
  })
  @ValidateNested()
  @Type(() => RoadMapUpdateManyWithoutStudentsInput)
  @IsOptional()
  @Field(() => RoadMapUpdateManyWithoutStudentsInput, {
    nullable: true,
  })
  roadMaps?: RoadMapUpdateManyWithoutStudentsInput;
}

export { StudentUpdateInput as StudentUpdateInput };
